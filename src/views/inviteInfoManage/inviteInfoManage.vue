<template>
	<section>
		...
		<el-table :data="tableData" highlight-current-row v-loading="listLoading"style="width: 100%;">
			...
			<el-table-column label="应聘" fixed="right" width="80" v-if="userType==2">
				<template scope="scope">
					<el-button size="small" type="info" @click="handleApply(scope.$index, scope.row)">应聘</el-button>
				</template>
			</el-table-column>
		...
		<el-dialog title="查询" v-model="inquiryFormVisible" :close-on-click-modal="false">
			<el-form :model="inquiryForm" label-width="90px" :rules="inquiryFormRules" ref="inquiryForm">
				<el-form-item label="招聘编号" prop="offerid">
					<el-input v-model.number="inquiryForm.offerid" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="企业名称" prop="sname">
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="90px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item label="所属行业" prop="manage">
					<el-select v-model="addForm.manage" clearable placeholder="请选择">
					    <el-option
					      v-for="item in manages"
					      :label="item.label"
					      :value="item.value"
...
//							alert("this.addForm="+JSON.stringify(this.addForm));
							...
							para.offerid = this.total+1;
							post('/api/offerjob/add',para).then((response) => {//请求成功
			...
			handleApply:function(index, row){
				this.$confirm('确认应聘吗？', '提示', {}).then(() => {
					NProgress.start();
					let para = {};
					para.offerid = row.offerid;
					para.job = row.job;
					para.sname = JSON.parse(sessionStorage.getItem('user')).name;//企业名字
					post('/api/person/employ',para).then((response) => {
						NProgress.done();
