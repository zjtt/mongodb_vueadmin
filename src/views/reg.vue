<template>
  <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-position="left" label-width="0px" class="demo-ruleForm login-container">
    ...
    <el-form-item prop="account">
      <el-input type="text" v-model="ruleForm2.account" auto-complete="off" placeholder="登录名"></el-input>
    </el-form-item>
      ...
      handleSubmit2(ev) {
        ...
        this.$refs.ruleForm2.validate((valid) => {
          if (valid) {
            ...
            this.logining = true;
            //NProgress.start();
            var regParams = { username: this.ruleForm2.account, password: this.ruleForm2.checkPass };
            if(this.ruleForm2.userType == 2){
	            let para = Object.assign({}, regParams);
	            para.age = '';
	            para.sex = '';
	            para.birthday = '';
	            para.school = '';
	            ...
	            para.email = '';
	            para.resume = '';
              post('/api/person/reg',para).then(data => {
...
//                alert(JSON.stringify(user));
	            ...
	            para.address = '';
	            para.cintro = '';
              post('/api/company/reg',para).then(data => {
//              alert(JSON.stringify(data.data));
                this.logining = false;
                //NProgress.done();
                let { msg, status, user } = data.data;
